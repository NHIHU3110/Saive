...
function renderProducts() {
  ...
  tbody.innerHTML = state.products.map(p => `
    <tr>
      ...
      <td style="font-weight:600;">${p.ProductName || p.name || 'N/A'}</td>
...
window.editProduct = function(id) {
  ...
  $('product-ProductName').value = p.ProductName || p.name || '';
  ...
  $('product-Description').value = p.Description || p.description || '';
...
window.openOrderCreateModal = function() {
  ...
  if (select) {
    select.innerHTML = '<option value="">-- Chọn sản phẩm --</option>' +
      state.products.map(p => `<option value="${p.id}">${p.ProductName || p.name} (${fmt(p.Price || p.price)})</option>`).join('');
  }
...
window.saveUser = async function() {
  ...
  try {
    if (id) await db.ref(`Users/${id}`).update(data);
    else await db.ref('Users').push(data);
...
window.saveEmployee = async () => {
  ...
  try {
    if (id) await db.ref(`Employees/${id}`).update(data);
    else await db.ref('Employees').push(data);
  ...
  tbody.innerHTML = state.blogs.map(b => {
    ...
    return `
      <tr data-date="${dateStr}">
        ...
        <td style="font-weight:600;">${b.Title || b.title || 'Không tiêu đề'}</td>
...
window.editVoucher = (id) => {
  ...
  $('voucher-description').value = v.description || v.Description || '';
...
window.saveVoucher = async () => {
  ...
  try {
    if (id) await db.ref(`Vouchers/${id}`).update(data);
    else await db.ref('Vouchers').push(data);
...
function renderReviews() {
  ...
  tbody.innerHTML = state.reviews.map(r => {
    ...
    <tr data-status="${isApproved ? 'true' : 'false'}" data-date="${dateStr}">
      <td>${r.ProductName || r.ProductId || 'Sản phẩm'}</td>
...
window.approveReview = async (id) => {
  try {
    await db.ref(`Reviews/${id}`).update({ IsApproved: true });
...
window.editBlog = function(id) {
  ...
  $('blog-Title').value = b.Title || b.title || '';
  ...
  $('blog-Content').value = b.Content || b.content || '';
  showModal('blog-modal');
}
...
window.saveBlog = async () => {
  ...
  try {
    if (id) await db.ref(`Blogs/${id}`).update(data);
    else await db.ref('Blogs').push(data);
...
window.filterOrders = () => {
  ...
  rows.forEach(row => {
    ...
    const rowDateStr = row.dataset.date; // Đã được format YYYY-MM-DD trong render
...
window.filterBlogs = () => {
  const query = $('blog-search').value.toLowerCase();
...
window.filterVouchers = () => {
  const query = $('voucher-search').value.toLowerCase();
