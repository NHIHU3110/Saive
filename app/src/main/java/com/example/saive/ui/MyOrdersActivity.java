...
public class MyOrdersActivity extends BaseActivity {
    ...
    private AdminOrder parseOrder(com.google.firebase.database.DataSnapshot child) {
        try {
            ...
            if (orderId == null) orderId = child.getKey();
            ...
            String shippingAddress = child.child("ShippingAddress").getValue(String.class);
            ...
            if (itemsSnapshot.exists()) {
                ...
                for (com.google.firebase.database.DataSnapshot itemChild : itemsSnapshot.getChildren()) {
                    String prodName = itemChild.child("ProductName").getValue(String.class);
                    Integer qty = itemChild.child("Quantity").getValue(Integer.class);
                    if (qty == null) qty = 1;
                    String size = itemChild.child("Size").getValue(String.class);
                    Object priceObj = itemChild.child("Price").getValue();
                    String img = itemChild.child("Image").getValue(String.class);

                    summary.append(qty).append("x ").append(prodName);
                    if (i < totalItemsCount - 1) summary.append(", ");
                    
                    items.add(new OrderItem(prodName, size != null ? size : "M", "Default", qty, String.valueOf(priceObj), R.drawable.tshirt1, img));
                    i++;
                }
            }
        ...
        List<AdminOrder> filteredList = allOrders.stream()
                .filter(o -> {
                    ...
                    if (tabPosition == 0) {
                        ...
                        return s.equals("PENDING") || s.equals("SHIPPED") || s.equals("IN PROGRESS");
                    }
                    if (tabPosition == 1) {
