                    ...
                    if (snapshot.exists()) {
                        ...
                        for (com.google.firebase.database.DataSnapshot child : snapshot.getChildren()) {
                            try {
                                ...
                                if (title == null) title = child.child("discountType").getValue(String.class);
                                
                                String desc = child.child("Description").getValue(String.class);
                                if (desc == null) desc = child.child("description").getValue(String.class);

                                // Lấy Discount an toàn (chấp nhận cả String và Number)
                                Object discountObj = child.child("Discount").getValue();
                                if (discountObj == null) discountObj = child.child("discount").getValue();
                                String discount = (discountObj != null) ? String.valueOf(discountObj) : "0";
                                
                                // Fix: Try both EndDate (used by Admin) and ExpiryDate (legacy)
                                String expiryDate = child.child("EndDate").getValue(String.class);
                                if (expiryDate == null) expiryDate = child.child("endDate").getValue(String.class);
                                if (expiryDate == null) expiryDate = child.child("ExpiryDate").getValue(String.class);
                                
                                String code = child.child("Code").getValue(String.class);
                                if (code == null) code = child.child("code").getValue(String.class);
                                
                                String status = child.child("Status").getValue(String.class);
                                if (status == null) status = child.child("status").getValue(String.class);

                                Integer points = child.child("PointsRequired").getValue(Integer.class);
                                if (points == null) points = 0;

                                serverCoupons.add(new Coupon(title, desc, discount, expiryDate, code, status, points));
                            } catch (Exception e) {
