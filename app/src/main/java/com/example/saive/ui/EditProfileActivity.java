            ...
            com.example.saive.admin.connectors.FirebaseConnector.getDatabase()
                    ...
                    .addListenerForSingleValueEvent(new com.google.firebase.database.ValueEventListener() {
                        ...
                        public void onDataChange(@androidx.annotation.NonNull com.google.firebase.database.DataSnapshot snapshot) {
                            ...
                            if (dob == null) dob = snapshot.child("DateOfBirth").getValue(String.class);
                            if (dob != null && !dob.isEmpty()) {
                                tvDob.setText(dob);
                                tvDob.setTextColor(getColor(R.color.colorNoirBlack));
                            }
                            String gender = snapshot.child("gender").getValue(String.class);
                            if (gender == null) gender = snapshot.child("Gender").getValue(String.class);
                            if (gender != null && !gender.isEmpty()) {
                                tvGender.setText(gender);
                                tvGender.setTextColor(getColor(R.color.colorNoirBlack));
                            }
                            String avatar = snapshot.child("avatarUrl").getValue(String.class);
                            if (avatar == null) avatar = snapshot.child("AvatarUrl").getValue(String.class);
    ...
    private void syncProfileToServer() {
        ...
        if (userId == null || userId.isEmpty()) {
            ...
            session.setCache(null, session.getEmail(), name, session.getRole(),
                    session.getAvatarUrl(), session.getPhone());
